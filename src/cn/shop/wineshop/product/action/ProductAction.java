...
public class ProductAction extends ActionSupport implements ModelDriven<Product> {
	...
	public String findByName() {
		Product exitproduct = productService.findByName(product.getPname());
		...
		if (exitproduct != null) {
			// 查询到该用户:用户名已经存在
			try {
				response.getWriter().println("<font color='red'>商品名称已经存在</font>");
			} catch (IOException e) {
			...
			// 没查询到该用户:用户名可以使用
			try {
				response.getWriter().println("<font color='green'>商品名称可以使用</font>");
