...
<script>
	function checkForm() {
		...
		if (username == "") {
			...
			xhr.onreadystatechange = function() {
				if (xhr.readyState == 4) {
					...
					}//if(xhr.responseText!="2"){return false;}		
				}
				// 3.打开连接
				xhr.open("GET",
		...
		if (email == "") {
			document.getElementById('span4').innerHTML = "<font color='red'>邮箱不能为空</font>";
			return false;
			...
			isok = reg.test(email);
			if (!isok) {
				document.getElementById('span4').innerHTML = "<font color='red'>邮箱地址不正确</font>";
				return false;
			}
		}
		//////////////////////////////校验手机号
		...
		isok = regphone.test(phone);
		if (phone == "")
			document.getElementById('span5').innerHTML = "<font color='green'>确定不输入电话吗？（非必填）</font>";

		else if (!isok) {
			document.getElementById('span5').innerHTML = "<font color='red'>号码格式不正确（非必填）</font>";
			return false;

		} else
			document.getElementById('span5').innerHTML = "<font color='green'>号码输入正确</font>";

		////////////////////////////////////////////校验地址

		var address = document.getElementById("address").value;
		if (address == "" || address == null) {
			document.getElementById('span6').innerHTML = "<font color='red'>地址不能为空</font>";
			return false;

		} else
			document.getElementById('span6').innerHTML = "<font color='green'>地址格式正确</font>";
	...
	function Password() {
		...
		if (password == "" || password == null) {
			...
			document.getElementById('span2').innerHTML = "<font color='green'>格式正确</font>";
	...
	function checkPassword() {
		...
		if (password != repassword) {
			document.getElementById('span3').innerHTML = "<font color='red'>两次输入密码不一致！</font>";
		} else if (repassword != "")

			document.getElementById('span3').innerHTML = "<font color='green'>输入正确</font>";
	}
	...
	function checkEmail() {
		...
		if (email == "") {
			...
			if (!isok) {
				...
				document.getElementById('span4').innerHTML = "<font color='green'>邮箱输入正确</font>";
		}
	}
	/////////////////////////////判断电话
	function checkphone() {
							...
							<tbody>
								...
								<tr>
									...
									<td><input type="password" id="password" name="password"
										...
										onblur="Password()" /> <span id="span2"><s:fielderror
								...
								<tr>
									...
									<td><input id="repassword" type="password"
										...
										autocomplete="off" onblur="checkPassword()" /> <span
										id="span3"></span></td>
								</tr>
								<tr>
									<th><span class="requiredField">*</span>E-mail:</th>
									<td><input type="text" id="email" name="email"
										class="text" maxlength="200" onblur="checkEmail()" /> <span
										id="span4"><s:fielderror fieldName="email" /></span></td>
								</tr>
								<tr>
									<th>姓名:</th>
								...
								<tr>
									...
									<td><input type="text" id="phone" name="phone"
										class="text" maxlength="200" onblur="checkphone()" /> <span
										id="span5"></span></td>
								</tr>

								<tr>
									<th>地址:</th>
									<td><input type="text" name="addr" id="address"
										class="text" maxlength="200" onblur="Address()" /> <span
										id="span6"><s:fielderror fieldName="addr" /></span></td>
								</tr>
								<tr>
